...
doc.css('div.span9 .levels a.level').map { |a| a.attributes['href'].value }.each do |row| 
  page = Nokogiri::HTML(open("https://www.memrise.com/" + row))
  page.css('div.container.container-main .central-column .things.clearfix .thing.text-text').each do |link|
